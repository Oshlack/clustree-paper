...
[@Thorndike1953-uv] or silhouette [@Rousseeuw1987-sm]) designed to help analysts
...
contains 2700 PBMCs representing a range of well-studied immune cell types
...
resolution parameter from zero to one (see methods). Seurat uses a graph-based
clustering algorithm and the resolution parameter controls the partitioning of
this graph, with higher values resulting in more clusters. The clustering tree
produced from this analysis is shown in Figure \@ref(fig:seurat-tree).
...
the Reingold-Tilford tree layout [@Reingold1981-iy] available as part of the
igraph package [@Csardi2006-ce].
