...
samples in largest cluster at $k = 2$ end up in the last cluster at $k = 3$.
In some datasets there may also be edges that contain few samples which are not
informative and result in a cluttered tree. The solution to this problem is to
remove some of the less important edges, with the obvious approach being to set
a threshold number of samples to remove low-count edges. In this case the count
of samples is not the correct statistic to use as it naturally favours edges at 
...
varying the clustering resolution parameter from zero to five (see methods).
...
clusters. The clustering trees produced from this analysis are shown in Figure
\@ref(fig:seurat-tree).
...
Looking at Figure \@ref(fig:seurat-tree)A which shows resolutions from zero to
one we see that four main branches quickly form in this tree at a resolution of 
just 0.1. One of these branches, starting with Cluster 3 at resolution 0.1
remains unchanged while the branch starting with Cluster 2 splits only once at a
...
begin to see more low in-proportion edges and new clusters forming from multiple
parent clusters. This suggests that those areas of the tree are unstable and
that the new clusters being formed there are unlikely to represent true groups
in the dataset.
