...
analysis of scRNA-seq data. Single-cell RNA-sequencing is a technology that
measures how genes are expressed in individual cells. This technology has been
rapidly adopted in fields like developmental biology where is valuable to have
information from single cells rather than the measurements that are averaged
across the many cells in a sample using older RNA sequencing technologies. One
of the key uses for scRNA-seq is to discover and interrogate the different cell
types present in a sample of a complex tissue. In this situation clustering is
typically used to group similar cells and genes that are differentially
expressed between groups are used to identify what processes those cells might
be involved. The number of cell types in an scRNA-seq dataset can vary depending
on factors such as the tissue being studied, it's developmental or environmental
state and the number of cells captured, therefore deciding which clustering
resolution to use is an important consideration in this kind of analysis.
...
exception of varying the clustering resolution parameter from zero to one.
Seurat uses a graph-based clustered algorithm and the resolution parameter
...
more expressed as we follow one of the central branches, allowing us which
...
\@ref(fig:seurat-genes)D), separating memory and naive CD4 T cells. While there
are alternative visualisations that can show differences in expression between
clusters by adding this information to a clustering tree we can see if more
populations can be identified as resolution is increased and if clusters are
split in biologically sensible ways.
...
which cluster are clearly separate and distinct, which are related to each other
and how sample change as more clusters are produced. Although clustering trees
can appear similar to the trees produced from hierarchical clustering there
are several important difference. Hierarchical clustering considers the
...
More experience with clustering trees may suggest other applications and they
could be adapted to be more flexible, such as allowing fuzzy clusterings.
