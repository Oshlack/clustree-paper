...
present here, without calculating scores: (i) a dataset is clustered at
multiple resolutions producing sets of cluster nodes, (ii) the overlap between
clusters at adjacent resolutions is used to build edges, (iii) the resulting
graph is presented as a tree. This tree can be used to examine how clusters are
...
single-cell RNA-sequencing (scRNA-seq) dataset. The figures shown here can be
produced in R using our publicly available clustree package. Although clustering
trees can not directly provide a clustering resolution to use they can be a
useful tool for exploring the range of possible choices.
...
examples using simple simulations. We present five scenarios: random uniform
noise, a single cluster, two clusters, three clusters and four clusters. Each
cluster consists of 1000 points generated from a 100 dimensional normal
distribution and each synthetic dataset has been clustered using $k$-means
clustering with $k = 1,...,8$. We then use the clustree package to produce
clustering trees for each dataset (Figure \@ref(fig:simulations)).
...
and where low in-proportion edges form we can infer which areas of the tree are
likely to be the result of true cluster and which are caused by
over-clustering.
...
multiple branches with more low proportion edges. As we have seen in the
simulated example this kind of pattern can indicate that the data has become
...
shows the relationships between clusterings at multiple resolutions. While
clustering trees cannot directly suggest which clustering resolution to use they
can be a useful for helping to make that decision, particularly when combined
with other metrics or domain knowledge.
...
To illustrate the uses of clustering trees we presented a series of
demonstration simulations and two examples of real analyses, one using the
classical iris dataset and a second based on a complex scRNA-seq dataset. Both
examples demonstrate how a clustering tree can help inform the decision of
which resolution to use and how overlaying extra information can help to
validate those clusters. This is of particular use to scRNA-seq analysis as
these datasets are often large, noisy and contain an unknown number of cell
types.
...
Each cluster consists of 1000 points sampled from a 100 dimensional space. The
first cluster was generated by selecting a centre in this space and generating
points normally distributed around this centre. The following clusters were
generated by applying a shift to an existing centre then generating 1000 points
in the same way. Each simulated dataset was clustered using the "kmeans"
function in the stats package with values of $k$ from one to eight, a maximum
of 100 iterations and 10 random starting positions. The clustering tree
visualisations were produced using the clustree package with the tree layout.
