...
While fuzzy or soft clustering assigns each sample to every cluster with
...
distances between samples (e.g. $k$-means [@Forgy1965-oj; @Macqueen1967-ag;
...
resolutions then calculates a similarity between clusterings at the same
...
another resolution but penalised if they are at a higher resolution.
...
produced in R using our publicly available clustree package.
...
edge to the higher resolution cluster independently of the cluster size. We
apply a threshold to the in-proportion in order to remove less informative
edges.
...
package [@Csardi2006-ce]. These are the Reingold-Tildord tree, layout which
...
clustering with $k = 1,...,8$. When then use the clustree package to produce
clustering trees for each dataset.
...
Looking at the first two examples (uniform noise and a single cluster) we can
clearly see how a clustering tree behaves when a clustering algorithm returns
more clusters than are truly present in a dataset. New clusters begin to form
from multiple existing clusters and many samples switch between branches of the
tree resulting in low in-proportion edges. Although not shown here
over-clustering with some algorithms may also cause clusters to appear then
disappear as the resolution increases. As we add more structure to the datasets
the clustering trees begin to form clear branches and low in-proportion edges
tend to be confined to sections of the tree. By looking at which clusters are
stable and where low in-proportion edges form we can infer which areas of the
tree are likely to be the result of true cluster and which are caused by
over-clustering.
...
The clustree package is available from GitHub at https://github.com/lazappi/clustree and the code and datasets used for the
