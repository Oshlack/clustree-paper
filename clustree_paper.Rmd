...
Clustering is widely used in the analysis of large data sets in order to group
together samples with similar properties. For example, clustering is being
widely used in the field of single-cell RNA-sequencing in order to identify
different cell types present in a tissue sample. Although there are many
algorithms to perform clustering the results can vary substantially. In
particular, the number of clusters present in a data set is often unknown and
can be varied based on parameters in the clustering algorithm. In order to
explore and examine the impact of varying clustering resolutions we present
clustering trees. This visualisation shows the relationships between clusters
at multiple resolutions allowing researchers to see how samples move as the
number of clusters increases. In addition, meta-information can be overlaid on
the tree in order to inform the choice of resolution resolution and guide in
...
variety of ways, such as by considering the distances between sample (e.g.
$k$-means [@Macqueen1967-ag], PAM [@Kaufman1990-mt]), areas of density across
the dataset (e.g. DBSCAN [@Ester1996-da]) or relationships to statistical
...
more important edges. By default, nodes are sized according to the number of
samples in the cluster they represent, with their colour indicating the
...
```{r seurat-tree, fig.cap = "Two clustering trees of a dataset of 2700 Peripheral Blood Mononuclear Cells (PBMCs). A) results from clustering using Seurat with resolution parameters from zero to one. At a resolution of 0.1 we see the formation four main branches, one of which continues to split up to a resolution of 0.5, after which there are only minor changes. B) resolutions from zero to five. At the highest resolutions we begin to see many low in-proportion edges indicating cluster instability. Seurat labels clusters according to their size with Cluster 0 being the largest."}
...
more flexible, such as accommodating fuzzy clusterings, and are likely to have
