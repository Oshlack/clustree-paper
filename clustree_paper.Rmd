...
While other forms of clustering exist, such as fuzzy or soft clustering where
each sample is assigned to every cluster with some probability, or hierarchical
clustering which forms a tree of samples, most methods form hard clusters where
each sample is assigned to a single group. This goal can be achieved in a
variety of ways, for example by considering the distances between sample
($k$-means [@Macqueen1967-ag], PAM [@Kaufman1990-mt]), areas of density across
the dataset (DBSCAN [@Ester1996-da]) or relationships to statistical
distributions [@Fraley2002-ps].
...
provided. Other methods have parameters that, directly or indirectly,
...
present here: (i) a dataset is clustered at multiple resolutions, (ii) the
overlap between clusters at adjacent resolutions is used to build edges, (iii)
the resulting graph is presented as a tree. This tree can be used to examine
how clusters are related to each other, which clusters are distinct and which
are unstable. In the following sections we describe how we construct such a
tree and present examples of trees built from a classical clustering dataset
and a complex single-cell RNA-sequencing (scRNA-seq) dataset. The figures shown
here can be produced in R using our publicly available clustree package.
...
samples is not the correct statistic to use as it naturally favours edges at
lower resolutions and those connecting larger clusters. Instead we define the
...
cluster size. We can apply a threshold to this value in order to remove less
informative edges.
...
more important edges. By default, nodes are sized according to the size of the
cluster they represent, with their colour indicating the resolution. The
clustree package also includes options for controlling the aesthetics of nodes
based on the attributes of samples in the clusters they represent.
...
causes particular clusters to split.
...
similar cells based on their gene expression. Differences in expression between
groups can then be used to infer the identity or function of those cells
[@Stegle2015-mc]. The number of cell types in an scRNA-seq dataset can vary
depending on factors such as the tissue being studied, its developmental or
environmental state and the number of cells captured. Often the number of cells
types is not known before the data is generated and some samples can contain
dozens of clusters. Therefore, deciding which clustering resolution to use is
...
consider commonly used Peripheral Blood Mononuclear Cell (PBMC) dataset. This
...
```{r seurat-tree, fig.cap = "Two clustering trees of a dataset of 2700 Peripheral Blood Mononuclear Cells (PBMCs). A) show results from clustering using Seurat with resolution parameters from zero to one. At a resolution of 0.1 we see the formation four main branches, one of which continues to split up to a resolution of 0.5, after which there are only minor changes. B) shows resolutions from zero to five. At the highest resolutions we begin to see many low in-proportion edges indicating cluster instability. Seurat labels clusters according to their size with Cluster 0 being the largest."}
...
The clustering tree covering resolutions zero to one (Figure
...
clusters are clearly separate and distinct, which are related to each other and
...
there are several important differences. Hierarchical clustering considers the
...
clusters established. In the future clustering trees could be adapted to be
more flexible, such as accommodating fuzzy clusterings, and may have
